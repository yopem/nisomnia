...
interface TranslateGenreFormProps {
  genreTranslationId: string
  language: LanguageType
  tmdbId: string
}

export default function TranslateGenreForm(props: TranslateGenreFormProps) {
  const { genreTranslationId, language, tmdbId } = props
  ...
  const { mutate: translateGenre } = api.genre.translate.useMutation({
    onSuccess: () => {
      form.reset()
  ...
  const onSubmit = (values: FormValues) => {
    ...
    setLoading(true)
    translateGenre(selectedFeaturedImage ? mergedValues : values)
    setLoading(false)
    router.push("/dashboard/genre")
  }
